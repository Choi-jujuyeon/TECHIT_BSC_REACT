...
router.post("/", async (req, res) => {
    try {
        ...
        if (!todo) {
            return res
                .status(400)
                .json({ ok: false, error: "Not exist todo." });
        }
        if (!userId) {
            return res
                .status(400)
                .json({ ok: false, error: "Not exist userId." });
        }
        ...
        if (!user) {
            return res
                .status(400)
                .json({ ok: false, error: "Not exist user." });
        }
...
router.get("/:userId", async (req, res) => {
    try {
        ...
        const todos = await client.todo.findMany({
            ...
            //get -- http://localhost:3010/todo/3?skip=15
        });
...
router.put("/:id/done", async (req, res) => {
    try {
        ...
        if (existTodo.userId !== parseInt(userId)) {
            return res
                .status(400)
                .json({ ok: false, error: "U R not todo owner." });
        }
